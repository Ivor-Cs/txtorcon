...
class EphemeralHiddenService(object):
    ...
    HS goes away. See documentation for ADD_EPH_HS (**XXX NOTE: this
    requires Yawning's branch feature6411 and shouldn't be used for
    anything but testing**)

    Quoting tor-dev post:
    ...
    def add_to_tor(self, protocol):
        ...
        '''
        ports = ' '.join(map(lambda x: x.strip(), self.ports))
        ans = yield protocol.queue_command('ADD_EPH_HS %s %s' % (self.key_blob, ports))
    ...
    def remove_from_tor(self, protocol):
        ...
        Returns a Deferred which fires with None
        '''
        r = yield protocol.queue_command('DEL_EPH_HS %s' % self.hostname[:-6])
        if r.strip() != 'OK':
